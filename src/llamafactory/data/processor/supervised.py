...
class SupervisedDatasetProcessor(DatasetProcessor):
    ...
    def _encode_data_example(
        ...
        encoded_pairs = self.template.encode_multiturn(self.tokenizer,
                                                       messages, system, tools)

        if hasattr(self.data_args,
                   'debug_template') and self.data_args.debug_template:
            import sys
            print(f"\n{'='*80}", file=sys.stderr)
            ...
            for i, msg in enumerate(messages):
                ...
                print(
                    f"  Message {i}: role={msg.get('role')}, content_len={len(msg.get('content', ''))}, has_tool_calls={has_tool_calls}",
                    file=sys.stderr)
            print(f"\nEncoded pairs count: {len(encoded_pairs)}",
                  file=sys.stderr)
            for i, (source_ids, target_ids) in enumerate(encoded_pairs):
                print(
                    f"  Pair {i}: source_len={len(source_ids)}, target_len={len(target_ids)}",
                    file=sys.stderr)
                # Check if this is a cumulative pair
                if len(target_ids) > len(source_ids) and target_ids[:len(
                        source_ids)] == source_ids:
                    print(f"    WARNING: Cumulative pair detected!",
                          file=sys.stderr)
                else:
                    print(f"    OK: Adjacent pair", file=sys.stderr)

        total_length = len(input_ids) + (1
                                         if self.template.efficient_eos else 0)
        ...
        for turn_idx, (source_ids, target_ids) in enumerate(encoded_pairs):
            ...
            if self.data_args.train_on_prompt:
                ...
                source_label = [self.tokenizer.eos_token_id
        ...
        if hasattr(self.data_args,
            ...
            print(f"train_on_prompt: {self.data_args.train_on_prompt}",
                  file=sys.stderr)
            print(f"mask_history: {self.data_args.mask_history}",
                  file=sys.stderr)
            ...
            print(
                f"IGNORE_INDEX count: {ignore_count} ({100*ignore_count/len(labels):.1f}%)",
                file=sys.stderr)
            print(
                f"Compute loss count: {compute_count} ({100*compute_count/len(labels):.1f}%)",
                file=sys.stderr)
            ...
            print(f"\nFirst 200 tokens mask pattern (I=IGNORE, C=COMPUTE):",
                  file=sys.stderr)
            mask_pattern = "".join([
                "I" if label == IGNORE_INDEX else "C" for label in labels[:200]
            ])
            ...
            if len(input_ids) <= 200:
                ...
                print(f"\nFirst 100 input_ids: {input_ids[:100]}",
                      file=sys.stderr)
    ...
    def preprocess_dataset(
            self, examples: dict[str, list[Any]]) -> dict[str, list[Any]]:
        # build inputs with format `<bos> X Y <eos>` and labels with format `<ignore> ... <ignore> Y <eos>`
        # for multiturn examples, we only mask the prompt part in each prompt-response pair.
        model_inputs = defaultdict(list)
        for i in range(len(examples["_prompt"])):
            if len(examples["_prompt"][i]) % 2 != 1 or len(
                    examples["_response"][i]) != 1:
                logger.warning_rank0("Dropped invalid example: {}".format(
            ...
            for i, length in enumerate(knapsack):
                ...
                if self.data_args.neat_packing:
                    packed_attention_masks += [i + 1] * len(
                        batch_input_ids[index])  # start from 1
                else:
                    packed_attention_masks += [1] * len(batch_input_ids[index])

            if len(
                    packed_input_ids
            ) < self.data_args.cutoff_len + 1:  # avoid flash_attn drops attn mask
                pad_length = self.data_args.cutoff_len - len(
                    packed_input_ids) + 1
