...
{{"name": <function-name>, "arguments": <args-json-object>}}
                    ...
                    # --- 3. Tool Calls 处理 ---
                    # 将 tool_calls 转换成 <tool_call> XML 格式并拼接到 content 末尾
                            ...
                            for tc_idx, tool_call in enumerate(tool_calls):
                                if isinstance(tool_call, str):
                                    try:
                                        ...
                                        logger.debug(
                                            f"Item {item_idx}, Message {msg_idx}, ToolCall {tc_idx}: Failed to parse as JSON"
                                        )
                                        continue
                                ...
                                if isinstance(tool_call, dict):
                                    ...
                                    if isinstance(func, str):
                                        try:
                                            ...
                                            logger.debug(
                                                f"Item {item_idx}, Message {msg_idx}: Failed to parse function as JSON"
                                            )
                                            continue
                                    ...
                                    if isinstance(func, dict):
                                        ...
                                        tool_call_obj = {
                                            "name": func_name,
                                            "arguments":
                                            func_args if isinstance(
                                                func_args, dict) else {}
                                        }
                                        tool_calls_xml.append(tool_call_obj)

                            # 将所有 tool_calls 转换为 XML 字符串并拼接到 content
                            if tool_calls_xml:
                                tool_calls_str = "\n".join(
                                    f'<tool_call>{json.dumps(tc, ensure_ascii=False)}</tool_call>'
                                    for tc in tool_calls_xml)
                                current_content = normalized_msg.get(
                                    "content", "")
                                if current_content:
                                    normalized_msg[
                                        "content"] = f"{current_content}\n{tool_calls_str}"
                                else:
                                    normalized_msg["content"] = tool_calls_str

                    # 处理 function_call（旧格式，转换为 <tool_call> XML）
                        ...
                        if isinstance(func_call, str):
                            try:
                                ...
                                logger.debug(
                                    f"Item {item_idx}, Message {msg_idx}: Failed to parse function_call as JSON"
                                )
                                func_call = {}
                        ...
                        if isinstance(func_call, dict):
                            ...
                            if isinstance(func_args, str):
                                func_args = _parse_arguments(func_args)

                            tool_call_obj = {
                                "name": func_name,
                                "arguments": func_args if isinstance(
                                    func_args, dict) else {}
                            }
                            tool_calls_str = f'<tool_call>{json.dumps(tool_call_obj, ensure_ascii=False)}</tool_call>'
...
if __name__ == "__main__":
    formatter = Formatter(input_dir="data/yulan_agent",
                          output_path="data/yulan.jsonl")
    formatter.format()
